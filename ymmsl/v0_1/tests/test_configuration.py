...
def test_configuration_update_implementations_override() -> None:
    ...
    base = PartialConfiguration(implementations=[implementation1, implementation2])
...
def test_configuration_update_checkpoint(test_config4: PartialConfiguration) -> None:
    # Note: test_checkpoint.py tests merging of checkpoint definitions
    base = PartialConfiguration(
        checkpoints=Checkpoints(wallclock_time=test_config4.checkpoints.wallclock_time)
    )
    overlay = PartialConfiguration(
...
def test_check_consistent(test_config6: Configuration) -> None:
    ...
    test_config6.implementations[Ref("c")].execution_model = ExecutionModel.DIRECT
    with pytest.raises(RuntimeError):
        test_config6.check_consistent()
    test_config6.implementations[Ref("c")].execution_model = ExecutionModel.OPENMPI
...
def test_load_implementations() -> None:
    ...
    assert configuration.implementations[Ref("micro")].script == ("/home/test/micro")
