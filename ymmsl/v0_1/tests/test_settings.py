...
def test_create_settings2() -> None:
    ...
    assert list(settings.ordered_items()[0][0]) == ["submodel", "_muscle_grain"]
...
def test_from_broken_dict() -> None:
    with pytest.raises(ValueError):
        Settings(OrderedDict([("$invalid&", 12)]))
...
def test_del_item(settings: Settings) -> None:
    settings._store = OrderedDict(
        [(Reference("param1"), "test"), (Reference("param2"), 0)]
    )
    del settings["param1"]
...
def test_load_settings() -> None:
    ...
    assert [str(s[0]) for s in settings.ordered_items()] == [
        ...
        "submodel1._muscle_timestep",
        "submodel1._muscle_total_time",
        "test_str",
        "test_int",
        "test_bool",
        "test_list_int",
