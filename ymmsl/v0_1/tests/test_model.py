...
def load_model() -> Callable:
    return yatiml.load_function(
        Model, Component, Conduit, Identifier, Ports, Reference, MulticastConduit
    )
...
def dump_model() -> Callable:
    return yatiml.dumps_function(
        Component, Conduit, Identifier, Model, Ports, Reference, MulticastConduit
    )
...
def macro_micro() -> Model:
    macro = Component(
        "macro", "my.macro", ports=Ports(o_i=["intermediate_state"], s=["state_update"])
    )
    micro = Component(
        "micro", "my.micro", ports=Ports(f_init=["initial_state"], o_f=["final_state"])
    )
...
model:
  ...
  conduits:
    ...
    model_overlay = Model("multicast", [], [Conduit("sender.port", "receiver2.port")])
