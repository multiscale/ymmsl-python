...
def test_config4() -> PartialConfiguration:
    implementations = [
        Implementation(Reference("isr2d.initial_conditions"), script="isr2d/bin/ic"),
        Implementation(Reference("isr2d.smc"), script="isr2d/bin/smc"),
        Implementation(Reference("isr2d.blood_flow"), script="isr2d/bin/bf"),
        Implementation(Reference("isr2d.smc2bf"), script="isr2d/bin/smc2bf.py"),
        Implementation(Reference("isr2d.bf2smc"), script="isr2d/bin/bf2smc.py"),
    ]
    resources = [
    ...
    return PartialConfiguration(
        None, None, implementations, resources, description, checkpoints, resume
    )
...
def test_config6() -> Configuration:
    ...
    implementations = [
        Implementation(Reference("a"), script="/home/user/models/bin/modela"),
        Implementation(Reference("b"), script="/home/user/models/bin/modelb"),
        Implementation(
            Reference("c"),
...
def test_config7() -> Configuration:
    model = Model(
        ...
        [
            Component(
                "macro", "macro_python", ports=Ports(o_i=["state_out"], s=["x_in"])
            ),
            Component(
                "micro",
                "micro_fortran",
                ports=Ports(f_init=["init_in"], o_f=["final_output", "extra_output"]),
            ),
        ],
        [
...
def test_config8() -> Configuration:
    model = Model(
        ...
        [
            Component(
                "macro",
                "macro_python",
                ports=Ports(o_i=["state_out1", "state_out2"], s=["x_in1", "x_in2"]),
            ),
            Component(
                "micro1",
                "micro1_python",
    ...
    implementations = [
        Implementation(
            Reference("macro_python"),
            base_env=BaseEnv.MANAGER,
            executable=Path("python"),
            args="macro.py",
        ),
        Implementation(
            Reference("micro1_python"),
            executable=Path("python"),
            args="micro1.py",
    ...
    resume = {Ref("macro"): Path("macro.pack"), Ref("micro1"): Path("micro1.pack")}
