...
def test_dump_custom_implementations(
    ...
    config_custom_implementations_text: str,
) -> None:
...
def test_dump_resources() -> None:
    resources = [ThreadedResReq(Ref("macro"), 10), ThreadedResReq(Ref("micro"), 1)]
...
def test_configuration_update_custom_implementations() -> None:
    base = Configuration(
        "Configuration for testing", custom_implementations={Ref("c1"): Ref("impl1")}
    )

    overlay1 = Configuration(
        "Extra and override",
        custom_implementations={Ref("c1"): Ref("impl2"), Ref("c2.a"): Ref("impl1")},
    )
...
def test_configuration_update_programs() -> None:
    ...
    overlay1 = Configuration(
        "Extra program", programs=[Program("impl2", executable=Path("impl2"))]
    )
    ...
    overlay2 = Configuration(
        "Conflicting program", programs=[Program("impl1", executable=Path("impl3"))]
    )
...
def test_configuration_update_checkpoint(
    config_update_checkpoint: Configuration,
) -> None:
...
def test_check_duplicate_implementations(
    config_duplicate_implementations: Configuration,
) -> None:
...
def test_check_component_loop(config_component_loop: Configuration) -> None:
    with pytest.raises(RuntimeError) as e:
...
def test_check_consistent_custom_implementations(
    config_consistent_custom_impls: Configuration,
) -> None:
    config_consistent_custom_impls.check_consistent()


def test_check_inconsistent_custom_impls(
    config_inconsistent_custom_impls: Configuration,
) -> None:
    with pytest.raises(RuntimeError) as e:
...
def test_check_inconsistent_resources(
    config_inconsistent_resources: Configuration,
) -> None:
    with pytest.raises(RuntimeError) as e:
...
def test_check_partial_resources(
    config_consistent_partial_resources: Configuration,
) -> None:
