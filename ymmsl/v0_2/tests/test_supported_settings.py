...
def test_create_supported_settings2(supported_settings: SupportedSettings) -> None:
    ...
    assert s[Identifier("text")].typ == SettingType.STR
    assert s[Identifier("text")].description == ""
    assert s[Identifier("alpha")].typ == SettingType.INT
    assert s[Identifier("beta")].typ == SettingType.FLOAT
    assert s[Identifier("enable_something")].typ == SettingType.BOOL
    assert s[Identifier("enable_something")].description == "Enables something"
    assert s[Identifier("things")].typ == SettingType.LIST_INT
    assert s[Identifier("numbers")].typ == SettingType.LIST_FLOAT
    assert s[Identifier("square")].typ == SettingType.LIST_LIST_FLOAT
    assert s[Identifier("square")].description == "Ensure it is not a rectangle"
...
def test_item_access(supported_settings: SupportedSettings) -> None:
    assert supported_settings["enable_something"].typ == SettingType.BOOL

    supported_settings["gamma"] = SupportedSetting("gamma", "[float]", "")
    assert supported_settings._store[Identifier("gamma")].typ == SettingType.LIST_FLOAT
    assert supported_settings[Identifier("gamma")].typ == SettingType.LIST_FLOAT

    supported_settings["gamma"] = SupportedSetting("gamma", "[int]", "description")
    assert supported_settings._store[Identifier("gamma")].typ == SettingType.LIST_INT
    assert supported_settings[Identifier("gamma")].description == "description"
...
def test_load(supported_settings: SupportedSettings, yaml_text: str) -> None:
    load = yatiml.load_function(
        SupportedSettings, Identifier, SettingType, SupportedSetting
    )
