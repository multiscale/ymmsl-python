...
def model() -> Model:
    return Model(
        ...
        [
            Component(
                "ic", Ports(o_f="out"), "Creates initial state", "initial_conditions"
            ),
            Component(
                "smc",
                Ports("initial_state", "cell_positions", "wss_in"),
                "Simulates smooth muscle cells",
                "smc",
            ),
            Component(
                "bf",
                Ports("initial_domain", o_f="wss_out"),
...
def model_with_filters() -> Model:
    return Model(
        ...
        [
            Component(
                "init",
                Ports(o_f="macro_out micro_out"),
                "Creates initial states",
                "init",
            ),
            Component(
                "macro1",
                Ports("init", "bc_out", "bc_in", "final"),
                "First macro model",
                "macro1",
            ),
            Component(
                "micro1",
                Ports("init_state init_bc", o_f="final_bc final_state"),
                "First micro model",
                "micro1",
            ),
            Component(
                "macro2",
                Ports("init_state", "bc_out", "bc_in"),
                "Second macro model",
                "macro2",
            ),
            Component(
                "micro2",
                Ports("init_state init_bc", o_f="final_bc"),
                "Second micro model",
                "micro2",
            ),
        ],
...
def test_program() -> Program:
    return Program(
        "macro",
        Ports(["init"], ["out1", "out2"], ["in1", "in2"], ["final"]),
        "description",
...
def config_update_checkpoint() -> Configuration:
    ...
    checkpoints = Checkpoints(
        True,
        [CheckpointRangeRule(every=100), CheckpointAtRule([10, 20, 50])],
        [
    ...
    resume = {
        Ref("ic"): Path("/path/to/snapshots/ic.pack"),
...
def config_duplicate_implementations() -> Configuration:
    ...
    return Configuration(
        "testing duplicate implementation names",
        None,
        [model1, model2],
        None,
        None,
...
def config_consistent_impl_ports() -> Configuration:
    model1 = Model(
        ...
        [
            Component(
                "no_implementation", Ports(o_i=["out"], s=["in"]), "description", None
            ),
            Component(
                "impl_no_ports",
    ...
    programs = [
        Program(
            "no_ports", None, "description", None, script="/home/user/models/bin/modela"
        ),
        Program(
            "with_ports",
            Ports(f_init=["in"], o_f=["out"]),
...
def config_inconsistent_impl_ports() -> Configuration:
    ...
    model2 = Model(
        ...
        [
            Component(
                "impl_also_wrong", Ports(o_i=["out"], s=["in"]), "description", "ports2"
            ),
            Component(
    ...
    programs = [
        Program("ports1", Ports(s=["test"]), script="/home/user/models/bin/modela"),
        Program(
            "ports2",
            Ports(f_init=["in"], o_f=["out"], s=["wrong"]),
            base_env=BaseEnv.LOGIN,
            modules=["gcc-6.3.0", "openmpi-1.10"],
            execution_model=ExecutionModel.OPENMPI,
    ...
    resources = [
        ...
        ThreadedResReq(Reference("implementations_test_broken.impl_ports_mismatch"), 1),
        MPICoresResReq(
            Reference("implementations_test_broken.model_as_impl.impl_also_wrong"), 4, 4
        ),
        ThreadedResReq(
            Reference("implementations_test_broken.model_as_impl.impl_extra_ports"), 1
        ),
    ]
...
def config_consistent_custom_impls() -> Configuration:
    model1 = Model(
        "test_model",
        None,
        "Testing consistent (custom) implementations",
        None,
        [
    ...
    return Configuration(
        "testing consistency of custom implementations",
        None,
        [model1, model2],
        {
...
def config_consistent_settings() -> Configuration:
    model1 = Model(
        ...
        [
            ...
            Component("submodel", Ports(), "description", "supported_settings_test2"),
        ],
    )
    ...
    program_a = Program(
        ...
        Ports(),
        "a",
        SupportedSettings({"alpha": "float", "beta": "int"}),
...
def config_consistent_resources() -> Configuration:
    ...
    return Configuration(
        "consistent_resources", None, [model1, model2], None, None, programs, resources
    )
...
def config_component_loop() -> Configuration:
    main = Model(
        ...
        [
            Component(
                "init", Ports(o_f="final"), "Calculates initial conditions", "submodel1"
            ),
            Component(
                "macro",
    ...
    submodel1 = Model(
        ...
        [
            Component(
                "first", Ports(f_init="init", o_f="final"), "First model", "submodel2"
            ),
            Component(
                "second",
                Ports(f_init="init", o_i="out", s="in"),
    ...
    return Configuration(
        "config_Component_loop",
        None,
        [main, submodel1, submodel2],
        None,
        None,
        programs,
        resources,
